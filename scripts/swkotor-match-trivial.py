...
DEFAULT_VC_ROOT = Path("/run/media/brunner56/MyBook/Toolchains/msvc8.0-main")
...
def vtable_tailcall_source(name: str, field_offset: int, method_offset: int) -> str:
    ...
    return "\n".join(
        [
            ...
            f"    (*(RecoveryVMethod **)target)[0][{slot}](target);",
            "}",
            "",
        ]
    )
...
def vtable_call_push_imm_source(name: str, method_offset: int, value: int) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"void __fastcall {name}(void *self) {{",
            f"    (*(RecoveryVMethodArg **)self)[0][{slot}](self, 0, {value});",
            "}",
            "",
        ]
    )
...
def import_call_ecx_source(name: str, address: int) -> str:
    return "\n".join(
        [
            "typedef void (__cdecl *RecoveryImportOneArg)(void *);",
            "",
            f"void __fastcall {name}(void *self) {{",
            f"    (*(RecoveryImportOneArg volatile *)0x{address:08x})(self);",
            "}",
            "",
        ]
    )
...
def import_call_imm_source(name: str, address: int, value: int) -> str:
    return "\n".join(
        [
            "typedef void (__cdecl *RecoveryImportIntArg)(int);",
            "",
            f"void {name}(void) {{",
            f"    (*(RecoveryImportIntArg volatile *)0x{address:08x})({value});",
            "}",
            "",
        ]
    )
...
def import_call_global_source(name: str, call_address: int, arg_address: int) -> str:
    return "\n".join(
        [
            "typedef void (__cdecl *RecoveryImportPtrArg)(void *);",
            "",
            f"void {name}(void) {{",
            f"    (*(RecoveryImportPtrArg volatile *)0x{call_address:08x})(*(void * volatile *)0x{arg_address:08x});",
            "}",
            "",
        ]
    )
