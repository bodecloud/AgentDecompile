"""Regression test: packaged-source candidates for __stdcall functions must
...
def test_packaged_stack_bytes_zero_for_void_params() -> None:
    # Name deliberately avoids an all-decimal hex suffix (e.g. "sub_1000")
    # -- that collides with a separate, pre-existing heuristic
    # (packaged_stack_bytes' `_(\d+)$` match against c_name) that
    # misinterprets an address-derived name as a stack-byte annotation.
    # Not this fix's concern; see the module docstring for the narrower
    # bug this test suite targets.
